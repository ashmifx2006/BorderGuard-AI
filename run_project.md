...
Copy the `token` value from the response, then create `ai_engine/.env` from `.env.example` and set:

```text
BORDERGUARD_API_TOKEN=paste-your-token-here
```
...
Copy `ai_engine/.env.example` to `ai_engine/.env` and set `BORDERGUARD_API_TOKEN` to the DRF token for the demo user. The AI engine no longer requires the token to be stored in `config.py`.
